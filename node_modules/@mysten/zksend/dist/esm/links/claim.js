...
var _client, _contract, _network, _host, _path, _claimApi, _gasCoin, _hasSui, _ownedObjects, _loadBagObject, loadBagObject_fn, _loadBag, loadBag_fn, _loadClaimedAssets, loadClaimedAssets_fn, _createSponsoredTransaction, createSponsoredTransaction_fn, _executeSponsoredTransaction, executeSponsoredTransaction_fn, _fetch, fetch_fn, _listNonContractClaimableAssets, listNonContractClaimableAssets_fn, _createNonContractClaimTransaction, createNonContractClaimTransaction_fn, _loadOwnedObjects, loadOwnedObjects_fn;
...
const _ZkSendLink = class {
  constructor({
    ...
    __privateAdd(this, _createSponsoredTransaction);
    __privateAdd(this, _executeSponsoredTransaction);
  ...
  async claimAssets(address, {
    ...
    const sponsored = await __privateMethod(this, _createSponsoredTransaction, createSponsoredTransaction_fn).call(this, tx, address, reclaim ? address : this.keypair.toSuiAddress());
    ...
    const { digest } = await __privateMethod(this, _executeSponsoredTransaction, executeSponsoredTransaction_fn).call(this, sponsored, signature);
  ...
  createClaimTransaction(address, {
    ...
    const [bag, proof] = reclaim ? __privateGet(this, _contract).reclaim(tx, { arguments: [store, this.address] }) : __privateGet(this, _contract).init_claim(tx, { arguments: [store] });
    const objectsToTransfer = [];
    const objects = [...this.assets?.coins ?? [], ...this.assets?.nfts ?? []];
    for (const object of objects) {
      objectsToTransfer.push(
        __privateGet(this, _contract).claim(tx, {
          arguments: [
            bag,
            proof,
            tx.receivingRef({
    ...
    __privateGet(this, _contract).finalize(tx, { arguments: [bag, proof] });
  ...
  async createRegenerateTransaction(sender, options = {}) {
    ...
    __privateGet(this, _contract).update_receiver(tx, { arguments: [store, this.address, to] });
    return {
      url: newLink.getLink(),
      transaction: tx
    };
...
_createSponsoredTransaction = new WeakSet();
...
_executeSponsoredTransaction = new WeakSet();
